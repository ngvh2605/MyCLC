...
const PersonalInfo: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (role == "teacher") {
      if (fullName && birth && gender && role && teacherSubject)
        setButtonDisabled(false);
      else setButtonDisabled(true);
    }
    if (role == "club") {
      if (fullName && birth && gender && role && clubCode && clubLink)
        setButtonDisabled(false);
      else setButtonDisabled(true);
    }
    if (role == "parent") {
      if (fullName && birth && gender && role && childName && childClass)
        setButtonDisabled(false);
      else setButtonDisabled(true);
    }
    if (role == "other") {
      if (fullName && birth && gender && role && other && otherPurpose)
        setButtonDisabled(false);
      else setButtonDisabled(true);
    }
  }, [
