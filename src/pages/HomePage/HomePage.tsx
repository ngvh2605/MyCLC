...
const HomePage: React.FC = () => {
  ...
  const oldTotalNews = usePrevious(totalNews);
  ...
  useEffect(() => {
    ...
    const snapshot = firestore.collection('news').onSnapshot(({ docs }) => {
  ...
  useEffect(() => {
    totalNews > oldTotalNews && oldTotalNews > 0 && setShowRefresh(true);
  }, [oldTotalNews, totalNews]);
  ...
  const fetchMore = async () => {
    const news = await getNextNews(lastKey, 1);
    setLastKey(news?.slice(-1)?.pop()?.timestamp || lastKey);
    ...
    <IonPage id='home-page'>
      ...
      <IonContent className='ion-padding'>
        ...
        <IonButton
          ...
          fill='clear'
          hidden={totalNews <= newsList.length}
          onClick={fetchMore}
        >
          <IonLabel>
            Đọc thêm
